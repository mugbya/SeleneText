...
1. 怎么判断主进程是否支持 ts
   ...
   * ts 跟 js 的区别
2. Electron 如何支持菜单中英文切换
3. Electron 能做哪些, 不能做哪些
4. 渲染进程 跟 主进程 怎么互相通信
5. 目录树的刷新与渲染，特别是操作目录树后，如果保证UI显示是及时且正确的
6. React 下 useState、useEffect 详细了解含意、作用及用法
   ...
   * useEffect: 副作用处理 Hook; 常用于：组件挂载后执行初始化逻辑、监听依赖变化、清理资源等
   ...
   ```
7. 怎么管理全局状态 Zustand
8. 怎么发现组建被重复渲染，然后怎么避免

* why-did-you-render 跟第三方组件容易冲突

* electron electron-devtools-installer 也一直不出现 Profiler

1. 怎么数据持久化，开发环境怎么清理这个持久化
2. 什么时候组件需要 props？
   ![](images/2.png)
3. zustand 的 set 的不同写法，推荐写法是什么

* set({ activeFile: file }) //  不需要返回

* set((state) => { ... }) // 必须返回

1. JavaScript 里面 .map  .filter .some 这三个方法分别是什么意思，作用是啥
...
1. 数据怎么持久化，有哪些方式，各自特点是什么，持久化的数据是保存在哪儿的
...
2. store 跟 hooks的区别是啥，怎么判断一个文件该放到store 还是放到hooks
...
5. 怎么在UI里面打开图片？
   ...
   * base64 太长，超出 URI 限制或内存上限

   * Blob URL

   * 还是要在index.html配置 CSP
